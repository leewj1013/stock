...
class IssueAlertTest(unittest.TestCase):
    ...
    def test_run_skips_when_no_issues(self, _issues, _env):
    ...
    def test_run_sends_when_issues_exist(self, _issues, send, _env):
    ...
    @patch("stock_alarm.issue_alert.issue_rows", return_value=[{"source": "run", "item": "positions_report", "status": "missing"}])
    def test_run_does_not_fail_daily_task_when_send_fails(self, _issues, _send, _env, log):
