...
class SellPerformanceTest(unittest.TestCase):
    ...
    def test_tracks_counterfactual_returns_after_sell(self, _execution, _close):
        ...
        self.assertEqual(110, rows[0]["entry_price"])
    ...
    @patch("stock_alarm.sell_performance.naver_close_after", return_value=100)
    @patch("stock_alarm.sell_performance.next_execution", return_value=(date(2026, 8, 3), 100))
    def test_deduplicates_same_sell_episode(self, _execution, _close):
        with tempfile.NamedTemporaryFile("w", delete=False, newline="", encoding="utf-8") as file:
            writer = csv.writer(file)
            writer.writerow(["created_at", "ticker", "name", "entry_price", "close"])
            writer.writerow(["2026-07-31T10:00:00", "005930", "Samsung", "110", "100"])
            writer.writerow(["2026-07-31T11:00:00", "005930", "Samsung", "110", "99"])
            path = file.name
        self.addCleanup(lambda: os.path.exists(path) and os.unlink(path))
        self.assertEqual(1, len(outcome_rows(path)))
