...
def outcome_rows(path: str = ALERT_PATH) -> list[dict]:
    ...
    for alert in alerts:
        ...
        if not created_at or not ticker:
            continue
        key = hashlib.sha256(f"{created_at}|{ticker}|{alert.get('entry_price', '')}".encode()).hexdigest()[:24]
        if key in seen:
            continue
        seen.add(key)
        ...
        row = {"alert_key": key, "alert_created_at": created_at, "ticker": ticker, "name": alert.get("name", ""), "alert_close": int(float(alert.get("close") or 0)), "execution_date": execution_day.isoformat() if execution_day else None, "execution_price": price, "execution_cost_bps": cost_bps, "updated_at": datetime.now().isoformat(timespec="seconds")}
